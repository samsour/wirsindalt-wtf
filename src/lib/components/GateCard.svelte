...
<style>
  ...
  .gate-bg  { min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 2rem; background: var(--paper); animation: card-up 0.5s ease-out backwards; }
  .gate-logo { width: 200px; height: auto; margin-bottom: .75rem; image-rendering: pixelated; animation: card-up 0.45s ease-out 0.1s backwards; }
  ...
  .gate-year  { font-size: 12px; letter-spacing: 2px; text-transform: uppercase; color: var(--accent); font-weight: 500; margin-bottom: 1rem; animation: card-up 0.4s ease-out 0.22s backwards; }
  .gate-title { font-family: var(--serif); font-size: 2rem; margin-bottom: .5rem; line-height: 1.2; animation: card-up 0.4s ease-out 0.32s backwards; }
  .gate-sub   { color: var(--ink2); font-size: 14px; margin-bottom: 1.5rem; animation: card-up 0.4s ease-out 0.42s backwards; }
  ...
  .gate-card input:nth-of-type(1) { animation: card-up 0.4s ease-out 0.52s backwards; }
  .gate-card input:nth-of-type(2) { animation: card-up 0.4s ease-out 0.62s backwards; }
  .gate-input:focus { border-color: var(--accent); }
  .gate-error { color: var(--red); font-size: 13px; margin: .5rem 0; }
  .gate-btn  { width: 100%; justify-content: center; margin-top: .5rem; animation: card-up 0.4s ease-out 0.72s backwards; }
  .gate-hint { font-size: 12px; color: var(--ink3); margin-top: 1rem; animation: card-up 0.4s ease-out 0.84s backwards; }
</style>
